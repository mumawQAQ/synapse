...
```bash
npm install @synapse/server @synapse/protocol socket.io openai zod
```
...
import { OpenAIAgentProvider } from "@synapse/server/provider";
...
```bash
npm install @synapse/client socket.io-client
```

Wrap your React app in the `<AgentProvider>`:

```tsx
import { AgentProvider } from "@synapse/client";
...
```tsx
import { useAgentTool } from "@synapse/client";
import { z } from "zod";
