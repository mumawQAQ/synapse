...
The `@mumaw/synapse-protocol` package contains strongly-typed interfaces and Zod schemas shared by both the Backend Server and the Frontend Client.
...
```typescript
import { ClientSideToolDefinition } from "@mumaw/synapse-protocol";
import { z } from "zod";
...
```typescript
import { ServerSideToolDefinition } from "@mumaw/synapse-protocol";
import { z } from "zod";
